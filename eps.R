...
# PDF settings
pdf("./datasets/Processed/EPS/alcohol_eps.pdf", width = 12, height = 8, compress = T)


# Create base chart
chart <- ggplot(eps, aes(x = eps_date)) +
    ggtitle("EPS Trend (Alcohol)") +
    theme(
...
# Write to CSV
write.csv(eps, "./datasets/Processed/EPS/alcohol_eps.csv", row.names = F)